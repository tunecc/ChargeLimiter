...
本文档记录用于生成安装包（TrollStore `.tipa`、roothide `.deb`、rootless `.deb`）的完整流程、易错点与踩坑修复，便于后续发布和复现。
...
> 说明：`<VERSION>` 以 `MARKETING_VERSION` 为准（当前为 1.9.5）。
...
- 生成 `.tipa` 前务必清理 `Payload` 里的 `.DS_Store`，否则会在安装时尝试写入 `/.DS_Store` 导致 rootless 报错：
...
可选：指定版本号（不传则自动读取 `MARKETING_VERSION`）：

```bash
./scripts/build_packages.sh 1.9.5
```

脚本会自动执行：
- rootless 构建（`arm64`）
- roothide 构建（`arm64e`）
- 对主程序和 daemon 执行 `strip` 后再签名（发布包更小）
- 生成 `.tipa`、`rootless .deb`、`roothide .deb`
- 校验 `CFBundleIdentifier`、entitlements（不允许 `no-container/no-sandbox`）、二进制架构

当前越狱包（`rootless/roothide`）卸载时会删除该应用的数据容器目录。

### 4.1 构建 App

```bash
rm -rf build Payload
xcodebuild -scheme "ChargeLimiter rootless" -configuration Release -derivedDataPath build CODE_SIGNING_ALLOWED=NO ARCHS=arm64
```

不要使用 `-scheme "ChargeLimiter"`，该 scheme 用于历史兼容，不作为发布构建入口。

手工流程下，建议分别构建 rootless 和 roothide，避免架构不一致。
...
#### 4.1.1 使用 DerivedData 的“最新产物”打包（避免老包）

如果你发现“代码改了但包里还是旧版本”，通常是因为打包时用了旧的 `build/` 目录。  
可以直接使用 Xcode 的 **DerivedData** 中最新生成的 `.app`：

```bash
APP="$HOME/Library/Developer/Xcode/DerivedData/ChargeLimiter-*/Build/Products/Release-iphoneos/ChargeLimiter.app"
```

后续打包命令里的 `build/Build/Products/...` 替换为 `$APP` 即可（见第 4.2~4.4）。
...
- **rootless / roothide 路径混用**：
  - roothide: `/var/jb/Applications/ChargeLimiter.app`
  - rootless: `/var/jb/Applications/ChargeLimiter.app`

- **rootless 脚本路径必须是 /var/jb**（不是 /Applications / /Library）：
  - `ChargeLimiter/Package/DEBIAN/postinst` / `prerm` / `postrm` 内路径需统一到 `/var/jb/...`
