...
- **锁屏后限流失效**：限流 thermal 写入增加读回校验——写入后比对 cltm 实际 pref，不一致记录 `thermal_write_unconfirmed` 事件；新增限流会话激活期间 60s 周期自愈定时器，在锁屏期电池事件稀疏、事件驱动 sync 存在空窗时兜底重写，避免 cltm 在锁屏期清除 thermalSimulationMode 后无自愈。
...
- **开机越狱后已插电时加速充电（LPM）不生效**：开机越狱后插电状态走不到任何命令翻转分支（插电事件/电量恢复/温控恢复），LPM 等加速项无路径开启。现充电稳态时重申 `performAcccharge(YES)`，由内部 `cache_status` 幂等守卫保证不重复写系统开关；同时修复该幂等守卫——避免每次电池事件重申时覆盖亮度缓存、重复写开关。
