...
- **电池兼容性测试页面**（主页面 → 更多功能 → 电池兼容性测试）：
  - **前置检查**：开始前校验已插电、daemon 在线，不满足则阻止并给出对应提示；当前未在充电（如已达充电目标或系统自行停充）时自动尝试恢复充电基线，基线无法建立（如接近满电）则中止并提示电量较低时重试。
  - **一键完整测试 / 单项选择**：默认依次执行停充、智能停充、禁流三项测试，每项上限 120 秒、判定已明确即提前结束；也可只勾选其中一项或多项重测。
  - **基线相对判定**：测试结论基于充电基线的相对变化，天然兼容「起始时未在充电」的状态；针对 iOS 17 下禁流写入被系统回写的对抗场景，信号恢复以 IsCharging 翻转为可靠判据并自动重试，识别插电状态来回翻转的异常循环，避免误判。
  - **实时进度与结果页**：每项测试实时显示执行状态与进度，结束后给出单项结论与总体判定，并提供页面内停充控制探针入口便于进一步手动验证；界面文案中英双语。
  - **配置自动恢复**：测试启动前快照全部相关配置，完成、取消、中途离开页面三条路径均自动恢复原配置，不残留测试写入。

### 改进

- README 整体重写为中英双语，新增电池兼容性测试、品牌新电池激活、充电宝兼容性等新用户章节（整理并扩充自上游文档）。

## v1.15.3 - 2026-08-28

### 修复

- **锁屏后限流自动失效，电流恢复到未限流水平（大概无效）**：v1.12.5 起 thermal mode 改为随电池/适配器实时读数推导（desired/sync 闭环），锁屏后 `ExternalChargeCapable` 塌陷、电流低于阈值，限流被系统读数推导回 off。三版补丁（读回校验/自愈/粘滞兜底）都在闭环内打转，方向反了。现整体退回原版命令驱动语义：thermal mode 只由 ChargeLimiter 自己的充电命令和配置决定，不读电池/适配器/电流/锁屏信号，锁屏天然不再改变限流状态。与原版（v1.7 系）锁屏行为一致。
- **限流配置切换不原子，两次 set_conf 间 thermal mode 可能不一致**：UI 切换限流等级原先发两次独立 `set_conf`（先 `adv_limit_inflow` 后 `adv_limit_inflow_mode`），两次之间 daemon 可能用半更新配置写 thermal，出现短暂错误档。新增原子 API `set_limit_inflow_config`：一次请求同时提交开关与等级，串行 handler 内批量写两键 + 一次 thermal 更新；写盘失败整组回滚不写 thermal。UI 改为调用单一原子方法。
- **iOS 17 禁流态亮屏误发"开始充电"通知（充电线未动）**：iOS 17 禁流改写 override key 后，`ExternalConnected`/`ExternalChargeCapable` 由系统间接派生，息屏周期性刷新抖动，被插电边沿判定当真插电，触发 `noti_start_charge`。现加入禁流态守卫：用户开关 `adv_disable_inflow=YES` 或上一轮 policy 处于禁流相关态（`no_inflow`/`temp_paused`）时，抑制派生值产生的伪插电边沿与 freshPlug 通知；真正拔线由 AdapterDetails 消失检测，拔线后守卫解除，后续真插电边沿正常产生。
...
- **iOS 17 停充后插线偶发不充电**：恢复充电增加 8 秒写后验证（电流 + `ChargingOverride` 位图），未生效自动重写一次，仍无效回退 legacy 写路径并记录诊断事件，修复硬件 inhibit 位图粘滞导致的「已连接电源 · 未充电」。
- **限流在未插电时持续生效、长时间后失效**：限流会话判定增加适配器连接前置条件，拔线时重置充电命令标志；未插电时恢复为默认模式（off），不再向 cltm 持续写模拟模式。
- **限流等级切换不实时**：限流相关四项配置改为 generation 去抖合并同步，双配置连发最终只写一次最终等级，消除中间态残留与 daemon 逐条同步竞态。
- **温控暂停时顶部电池图标跳变**：温度在停充上限附近振荡时，状态会在 `temp_paused` 与 `stopped`/`capacity_high` 间每个电池事件翻转一次。现加入滞回——温度未降到恢复线以下前，归因保持 `temp_paused`（`temperature_hysteresis`），两条路径充电行为本就一致（都停充），修正后只状态归因/显示不再跳。
- **开机越狱后已插电时加速充电（LPM）不生效**：开机越狱后插电状态走不到任何命令翻转分支（插电事件/电量恢复/温控恢复），LPM 等加速项无路径开启。改为 daemon `serve()` 末尾补一次策略刷新，让进入充电态的分支首次应用加速项，替代旧的稳态重申首次应用；同时引入会话级标志 `g_accChargeAppliedThisSession` 把「首次应用」与「稳态重申」职责分离，稳态重申仅恢复被外部清除的加速项，不再在 app/apply_now 触发策略时误首次应用（即「开 app 秒进低电量模式」）。
- **去抖 sync 的 dispatch_source 闭包捕获首次 generation 的 bug**：改用逐次 `dispatch_after`，同步用局部电池快照，避免后台线程写共享 `bat_info`。

### 改进

- 主页高温模拟卡片随电池轮询实时刷新：`get_bat_info` 附带 `ThermalSimulateMode`，限流状态变化在主界面即时反映，无需手动刷新。
