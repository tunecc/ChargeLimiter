...
禁流测试 SHALL 按以下流程执行：临时关闭 CL 全局开关，调用禁流接口（`set_inflow_status`，flag=false），随后持续监测电池状态与电流，监测窗口上限 120 秒。判定规则：

- 若监测窗口内禁流状态发生预期变化（充电停止/电流特征改变），SHALL 判定"支持禁流"；
- 若禁流后电池电流持续 ≥5mA，SHALL 判定"无法支持禁流"。

#### Scenario: 禁流生效判定为支持

- **WHEN** 禁流指令下发后电池电流特征在监测窗口内出现预期的放电/停止充电变化
- **THEN** 该项测试得出"支持禁流"结论

#### Scenario: 禁流后电流持续判定为不支持

- **WHEN** 禁流指令下发后电池电流持续 ≥5mA 无变化
