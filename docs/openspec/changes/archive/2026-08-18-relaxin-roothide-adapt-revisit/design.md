...
- [Relaxin 开源快照不等于实际装机版本] → 适配结论需在真机 Relaxin 上回归验证；源码层面确认正确不等于装机一定无问题。验收场景必须包含真机。
- [bootstrap tarball 内容不可见] → libroot.dylib / libroothide.dylib 的实际发布路径只能靠 finalizer 源码推断，装机时验证。
- [cfprefsd 重定向 + ChargeLimiter 自管 prefs 路径] → 两者目标一致但若有一方路径算错会互相覆盖。验收"改设置→重启→设置还在"必须覆盖。
- [pathhook 只 hook `__CFCopyHomeDirURLForUser`] → 已排查（见 D6）：ChargeLimiter 所有 `/var/mobile/...` 均走 jbroot API 解析，`NSHomeDirectory()` 走 pathhook，无硬拼绕过。风险消除。
- [root persona spawn 126 根因未在源码层面 100% 定位] → 既有"避开 root persona"修复有效但根因解释停留在现象层。不阻塞，但记录为 Open Question。

## Migration Plan

- 不涉及数据迁移。用户从 v1.15.x 升级到本次修正版：卸载旧版 → 装新版 → 重启 → 验收。若复核中发现需要改 entitlement/路径，需卸载重装（prerm 已有数据清理兜底）。
- 回滚：卸载新版，装回 v1.15.x deb。

## Open Questions

- Relaxin 下 root persona spawn exec 126 的**精确源码根因**（是 `systemwide_persona_fix` 的直接子进程校验？还是 spawn_hook 的 persona 处理？）。不阻塞本次适配（既有修复有效），但值得在 design/build 阶段顺手查清。
- Relaxin 实际装机时 `/var/jb/usr/lib/libroot.dylib` 与 `libroothide.dylib` 是否就位。装机期验证。

---

**复核结论摘要**：v1.15.0 的摸黑适配在 Relaxin 源码层面**基本正确**（D1-D4 保留），D6 排查通过无硬拼，没有发现"明显做错需要推倒重来"的点。本次 change 的实际产出预期是：

1. 一份源码复核报告（本文 design.md 即是）
2. 针对 D5 的验收确认（可能为零代码改动，仅真机回归）
3. 真机验收清单与回归

是否需要写 delta spec，取决于 D5 验收是否触及 spec 级行为变更 —— 默认 `skip_specs: true`，验收完成后再定。
